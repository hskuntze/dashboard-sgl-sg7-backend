...
public class OMServico {
	...
	/**
	 * Busca por uma entidade OM através do código dela
	 *
	 * @param id
	 * @return
	 */
	@Transactional(readOnly = true)
	public OM getEntityById(Long id) {
		OM om = omRepositorio.findById(id)
				.orElseThrow(() -> new RecursoNaoEncontradoException("Não foi possível encontrar uma OM com ID " + id));
		return om;
	}
